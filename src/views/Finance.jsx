              ...
              return (
                <div className={`${f.splitMaterialLabor ? "items-row-split" : "items-row"} ${it.isSub ? "items-row-sub" : ""}`} key={it.id}>
                  ...
                  <textarea
                    ...
                    value={it.desc}
                    onChange={(e) => setItem(it.id, "desc", e.target.value)}
                    placeholder={it.isSub ? "รายละเอียดย่อยของรายการด้านบน เช่น เทคอนกรีต, ผูกเหล็ก" : "เช่น งานปรับพื้นที่ดินและปลูกหญ้าบริเวณหน้าโรงงาน งวดที่ 1 เมื่อได้รับใบสั่งซื้อ เบิก 20% (มี PO พิมพ์ต่อท้ายในช่องนี้ได้เลย)"}
                  />
